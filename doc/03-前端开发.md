# 💻 前端开发

本文档详细介绍 MDtranslator 前端的架构设计、组件实现和开发指南。

## 目录

- [技术栈](#技术栈)
- [项目结构](#项目结构)
- [核心组件](#核心组件)
- [状态管理](#状态管理)
- [自定义 Hooks](#自定义-hooks)
- [API 层](#api-层)
- [样式系统](#样式系统)
- [开发指南](#开发指南)

---

## 技术栈

| 技术 | 用途 | 文档 |
|:---|:---|:---|
| Next.js 14 | 应用框架 | [nextjs.org](https://nextjs.org/docs) |
| React 18 | UI 库 | [react.dev](https://react.dev) |
| TypeScript | 类型系统 | [typescriptlang.org](https://www.typescriptlang.org/docs/) |
| Zustand | 状态管理 | [zustand](https://zustand-demo.pmnd.rs/) |
| CodeMirror 6 | 代码编辑器 | [codemirror.net](https://codemirror.net/) |
| react-markdown | Markdown 渲染 | [remarkjs](https://remarkjs.github.io/react-markdown/) |
| Framer Motion | 动画库 | [framer.com/motion](https://www.framer.com/motion/) |
| Tailwind CSS | CSS 框架 | [tailwindcss.com](https://tailwindcss.com/docs) |

---

## 项目结构

```
src/src/
├── app/                        # Next.js App Router
│   ├── page.tsx               # 首页入口
│   ├── layout.tsx             # 根布局
│   ├── globals.css            # 全局样式
│   └── api/                   # API Routes
│       ├── documents/         # 文档 API 代理
│       │   ├── route.ts       # GET/POST /api/documents
│       │   └── [id]/route.ts  # GET/DELETE /api/documents/:id
│       ├── settings/route.ts  # 设置 API 代理
│       ├── translate/route.ts # 翻译 API 代理
│       └── example/           # 示例文件 API
│           ├── route.ts       # 加载示例 README
│           └── assets/        # 静态资源
│
├── components/                 # React 组件
│   ├── SplitLayout.tsx        # 主布局组件（核心）
│   ├── Editor.tsx             # 代码编辑器
│   ├── Preview.tsx            # Markdown 预览
│   ├── UploadZone.tsx         # 文件上传
│   ├── Mermaid.tsx            # Mermaid 图表渲染
│   ├── layout/                # 布局组件
│   │   ├── index.ts           # 导出入口
│   │   └── Header.tsx         # 顶部工具栏
│   ├── modals/                # 弹窗组件
│   │   ├── index.ts
│   │   ├── SettingsModal.tsx  # 设置弹窗
│   │   └── HistoryModal.tsx   # 历史记录弹窗
│   ├── panels/                # 面板组件
│   │   ├── index.ts
│   │   ├── ContentPanel.tsx   # 内容面板
│   │   └── CollapsedPanel.tsx # 折叠面板
│   └── ui/                    # 基础 UI 组件
│       └── button.tsx
│
├── hooks/                      # 自定义 Hooks
│   ├── index.ts               # 导出入口
│   ├── useTranslation.ts      # 翻译逻辑
│   ├── useDocumentHistory.ts  # 历史记录管理
│   └── useSettings.ts         # 设置管理
│
├── store/                      # Zustand 状态
│   └── useDocumentStore.ts    # 文档状态存储
│
└── lib/                        # 工具函数
    └── utils.ts               # cn() 等工具
```

---

## 核心组件

### SplitLayout - 主布局组件

`SplitLayout` 是应用的核心组件，管理整体布局和交互逻辑。

**职责：**
- 管理双栏/四栏布局切换
- 协调各子组件状态
- 处理面板展开/折叠
- 集成翻译、历史、设置功能

```tsx
// src/components/SplitLayout.tsx
"use client";

import React, { useState, useCallback } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { useDocumentStore } from '@/store/useDocumentStore';
import { useTranslation, useDocumentHistory, useSettings } from '@/hooks';

export default function SplitLayout() {
  const { rawContent, translatedContent, layoutMode } = useDocumentStore();
  
  // Custom hooks
  const { isTranslating, startTranslation } = useTranslation();
  const { historyDocs, loadDocument, openHistory } = useDocumentHistory();
  const { settings, openSettings } = useSettings();

  // Panel state
  const [leftExpanded, setLeftExpanded] = useState(null);
  const [rightExpanded, setRightExpanded] = useState(null);

  // 渲染逻辑...
}
```

**布局模式：**

| 模式 | 描述 | 何时使用 |
|:---|:---|:---|
| `split` | 双栏布局（编辑器 + 预览） | 输入/查看原文时 |
| `quad` | 四栏布局（原文编辑器/预览 + 译文编辑器/预览） | 翻译过程中/翻译完成后 |

---

### Editor - 代码编辑器

基于 CodeMirror 6 的 Markdown 编辑器。

```tsx
// src/components/Editor.tsx
import CodeMirror from '@uiw/react-codemirror';
import { markdown, markdownLanguage } from '@codemirror/lang-markdown';
import { languages } from '@codemirror/language-data';

interface EditorProps {
  value: string;
  onChange?: (val: string) => void;
  readOnly?: boolean;
}

export default function Editor({ value, onChange, readOnly = false }: EditorProps) {
  return (
    <div style={{ height: '100%', width: '100%', overflow: 'hidden' }}>
      <CodeMirror
        value={value}
        height="100%"
        extensions={[
          markdown({ base: markdownLanguage, codeLanguages: languages })
        ]}
        onChange={(val) => onChange?.(val)}
        readOnly={readOnly}
        theme="light"
        style={{ height: '100%', fontSize: '14px' }}
      />
    </div>
  );
}
```

**特性：**
- Markdown 语法高亮
- 代码块语言识别
- 只读模式支持
- 自适应高度

---

### Preview - Markdown 预览

使用 react-markdown 渲染 Markdown 内容，支持多种扩展。

```tsx
// src/components/Preview.tsx
import ReactMarkdown from 'react-markdown';
import remarkGfm from 'remark-gfm';
import remarkMath from 'remark-math';
import rehypeKatex from 'rehype-katex';
import rehypeRaw from 'rehype-raw';

export default function Preview({ content }: { content: string }) {
  return (
    <div className="markdown-preview">
      <ReactMarkdown
        remarkPlugins={[remarkGfm, remarkMath]}
        rehypePlugins={[rehypeRaw, rehypeKatex]}
        skipHtml={false}
        components={{
          // 自定义组件渲染...
          code({ node, inline, className, children, ...props }) {
            const match = /language-(\w+)/.exec(className || '');
            // Mermaid 图表
            if (!inline && match?.[1] === 'mermaid') {
              return <Mermaid chart={String(children)} />;
            }
            // 代码块
            return <code className={className} {...props}>{children}</code>;
          },
          // 其他自定义组件...
        }}
      />
    </div>
  );
}
```

**支持的语法扩展：**

| 插件 | 功能 |
|:---|:---|
| `remarkGfm` | GitHub Flavored Markdown（表格、删除线等） |
| `remarkMath` | 数学公式语法解析 |
| `rehypeKatex` | KaTeX 数学公式渲染 |
| `rehypeRaw` | 支持内嵌 HTML |

---

### UploadZone - 文件上传

支持拖拽上传和点击选择的文件上传组件。

```tsx
// src/components/UploadZone.tsx (简化)
export default function UploadZone() {
  const { setRawContent } = useDocumentStore();

  const handleFile = async (file: File) => {
    const text = await file.text();
    setRawContent(text);
  };

  return (
    <div 
      onDrop={handleDrop}
      onDragOver={(e) => e.preventDefault()}
      className="upload-zone"
    >
      <p>拖拽 Markdown 文件到此处</p>
      <p>或点击选择文件</p>
      <button onClick={loadExample}>加载示例</button>
    </div>
  );
}
```

---

### Mermaid - 图表渲染

渲染 Mermaid 图表的专用组件。

```tsx
// src/components/Mermaid.tsx
import mermaid from 'mermaid';
import { useEffect, useRef } from 'react';

export default function Mermaid({ chart }: { chart: string }) {
  const ref = useRef<HTMLDivElement>(null);

  useEffect(() => {
    if (ref.current) {
      mermaid.init(undefined, ref.current);
    }
  }, [chart]);

  return <div ref={ref} className="mermaid">{chart}</div>;
}
```

---

## 状态管理

使用 Zustand 进行全局状态管理，简洁且高效。

### useDocumentStore

```tsx
// src/store/useDocumentStore.ts
import { create } from 'zustand';

export interface TranslationChunk {
  id: number;
  chunkIndex: number;
  rawText: string;
  translatedText: string | null;
  status: 'pending' | 'processing' | 'completed' | 'error';
}

interface DocumentState {
  // 状态
  docId: string | null;
  rawContent: string;
  translatedContent: string;
  chunks: TranslationChunk[];
  isTranslating: boolean;
  layoutMode: 'split' | 'quad';
  
  // 动作
  setDocId: (id: string) => void;
  setRawContent: (content: string) => void;
  setTranslatedContent: (content: string) => void;
  setChunks: (chunks: TranslationChunk[]) => void;
  updateChunk: (index: number, data: Partial<TranslationChunk>) => void;
  setIsTranslating: (isTranslating: boolean) => void;
  setLayoutMode: (mode: 'split' | 'quad') => void;
}

export const useDocumentStore = create<DocumentState>((set) => ({
  // 初始状态
  docId: null,
  rawContent: '',
  translatedContent: '',
  chunks: [],
  isTranslating: false,
  layoutMode: 'split',

  // 动作实现
  setDocId: (id) => set({ docId: id }),
  setRawContent: (content) => set({ rawContent: content }),
  setTranslatedContent: (content) => set({ translatedContent: content }),
  setChunks: (chunks) => set({ chunks }),
  
  // 更新单个分块并重新组装译文
  updateChunk: (index, data) => set((state) => {
    const newChunks = [...state.chunks];
    const chunkIdx = newChunks.findIndex(c => c.chunkIndex === index);
    if (chunkIdx !== -1) {
      newChunks[chunkIdx] = { ...newChunks[chunkIdx], ...data };
    }
    
    // 重新组装完整译文
    const assembled = newChunks
      .sort((a, b) => a.chunkIndex - b.chunkIndex)
      .map(c => c.translatedText || '')
      .join('');

    return { chunks: newChunks, translatedContent: assembled };
  }),
  
  setIsTranslating: (isTranslating) => set({ isTranslating }),
  setLayoutMode: (mode) => set({ layoutMode: mode }),
}));
```

**状态说明：**

| 状态 | 类型 | 说明 |
|:---|:---|:---|
| `docId` | `string \| null` | 当前文档 ID |
| `rawContent` | `string` | 原始 Markdown 内容 |
| `translatedContent` | `string` | 翻译后的内容 |
| `chunks` | `TranslationChunk[]` | 分块翻译数据 |
| `isTranslating` | `boolean` | 是否正在翻译 |
| `layoutMode` | `'split' \| 'quad'` | 布局模式 |

---

## 自定义 Hooks

### useTranslation - 翻译逻辑

封装完整的翻译流程，包括 WebSocket 通信。

```tsx
// src/hooks/useTranslation.ts
export function useTranslation() {
  const wsRef = useRef<WebSocket | null>(null);
  const { rawContent, setChunks, updateChunk, setIsTranslating } = useDocumentStore();

  const startTranslation = useCallback(async () => {
    setIsTranslating(true);

    // 1. 创建翻译任务
    const response = await fetch('/api/translate', {
      method: 'POST',
      body: JSON.stringify({ content: rawContent, title: '...' })
    });
    const data = await response.json();
    setChunks(data.chunks);

    // 2. 建立 WebSocket 连接
    const ws = new WebSocket(`ws://127.0.0.1:8000/ws/translate/${data.docId}`);
    wsRef.current = ws;

    // 3. 处理实时消息
    ws.onmessage = (event) => {
      const msg = JSON.parse(event.data);
      if (msg.type === 'chunk_update') {
        updateChunk(msg.chunkIndex, {
          translatedText: msg.data.translatedText,
          status: msg.data.status
        });
      } else if (msg.type === 'complete') {
        setIsTranslating(false);
      }
    };
  }, [rawContent]);

  return { isTranslating, startTranslation, closeWebSocket };
}
```

---

### useSettings - 设置管理

```tsx
// src/hooks/useSettings.ts
export interface Settings {
  llm_provider: string;
  llm_model: string;
  temperature: number;
  num_chunks: number;
  auto_save: boolean;
}

export function useSettings() {
  const [settings, setSettings] = useState<Settings>(defaultSettings);
  const [showSettings, setShowSettings] = useState(false);

  const loadSettings = useCallback(async () => {
    const res = await fetch('/api/settings');
    const data = await res.json();
    setSettings(data);
  }, []);

  const saveSettings = useCallback(async () => {
    await fetch('/api/settings', {
      method: 'POST',
      body: JSON.stringify({ settings })
    });
  }, [settings]);

  useEffect(() => { loadSettings(); }, []);

  return { settings, updateSettings, saveSettings, showSettings, openSettings, closeSettings };
}
```

---

### useDocumentHistory - 历史记录

```tsx
// src/hooks/useDocumentHistory.ts
export function useDocumentHistory() {
  const [historyDocs, setHistoryDocs] = useState<HistoryDoc[]>([]);
  const { setRawContent, setTranslatedContent, setChunks } = useDocumentStore();

  const loadHistory = useCallback(async () => {
    const res = await fetch('/api/documents');
    const data = await res.json();
    setHistoryDocs(data.documents);
  }, []);

  const loadDocument = useCallback(async (id: string) => {
    const res = await fetch(`/api/documents/${id}`);
    const doc = await res.json();
    setRawContent(doc.original_content);
    setTranslatedContent(doc.translated_content);
    setChunks(doc.chunks_data);
  }, []);

  const deleteDocument = useCallback(async (id: string) => {
    await fetch(`/api/documents/${id}`, { method: 'DELETE' });
    loadHistory();
  }, []);

  return { historyDocs, loadDocument, deleteDocument, openHistory, closeHistory };
}
```

---

## API 层

前端使用 Next.js API Routes 作为代理层，转发请求到后端。

### 配置代理

```javascript
// src/next.config.mjs
const nextConfig = {
  async rewrites() {
    return [
      { source: '/api/documents/:path*', destination: 'http://127.0.0.1:8000/api/documents/:path*' },
      { source: '/api/translate/:path*', destination: 'http://127.0.0.1:8000/api/translate/:path*' },
      { source: '/api/settings/:path*', destination: 'http://127.0.0.1:8000/api/settings/:path*' },
      { source: '/ws/:path*', destination: 'http://127.0.0.1:8000/ws/:path*' },
    ];
  },
};
```

### 禁用缓存

为确保获取最新数据，API Routes 需要禁用缓存：

```typescript
// src/app/api/documents/route.ts
export const dynamic = 'force-dynamic';
export const revalidate = 0;

export async function GET() {
  const response = await fetch('http://127.0.0.1:8000/api/documents', {
    cache: 'no-store',  // 关键：禁用缓存
  });
  // ...
}
```

---

## 样式系统

### Tailwind CSS 配置

```typescript
// src/tailwind.config.ts
import type { Config } from 'tailwindcss';

const config: Config = {
  content: [
    './src/**/*.{js,ts,jsx,tsx,mdx}',
  ],
  theme: {
    extend: {
      colors: {
        // 自定义颜色
      },
    },
  },
  plugins: [],
};
```

### 全局样式

```css
/* src/app/globals.css */

/* Markdown 预览样式 */
.markdown-preview {
  @apply p-4 prose prose-sm max-w-none;
}

.markdown-preview .md-h1 { @apply text-2xl font-bold mt-6 mb-4; }
.markdown-preview .md-h2 { @apply text-xl font-semibold mt-5 mb-3; }
.markdown-preview .md-p { @apply my-3 leading-relaxed; }
.markdown-preview .md-ul { @apply list-disc pl-6 my-3; }
.markdown-preview .md-ol { @apply list-decimal pl-6 my-3; }
.markdown-preview .md-blockquote { @apply border-l-4 border-gray-300 pl-4 italic; }
.markdown-preview .md-table { @apply w-full border-collapse; }
.markdown-preview .md-th { @apply border px-3 py-2 bg-gray-100; }
.markdown-preview .md-td { @apply border px-3 py-2; }
```

---

## 开发指南

### 添加新组件

1. 在 `src/components/` 下创建组件文件
2. 使用 TypeScript 定义 Props 接口
3. 如需全局状态，使用 `useDocumentStore`
4. 如需复杂逻辑，抽取到 Hooks

```tsx
// src/components/MyComponent.tsx
import { useDocumentStore } from '@/store/useDocumentStore';

interface MyComponentProps {
  title: string;
  onAction?: () => void;
}

export default function MyComponent({ title, onAction }: MyComponentProps) {
  const { rawContent } = useDocumentStore();
  
  return (
    <div className="my-component">
      <h2>{title}</h2>
      <button onClick={onAction}>执行</button>
    </div>
  );
}
```

### 添加新 Hook

1. 在 `src/hooks/` 下创建 Hook 文件
2. 在 `src/hooks/index.ts` 中导出
3. 遵循 `useXxx` 命名约定

```typescript
// src/hooks/useMyFeature.ts
export function useMyFeature() {
  const [state, setState] = useState();
  
  const doSomething = useCallback(async () => {
    // 实现逻辑
  }, []);
  
  return { state, doSomething };
}

// src/hooks/index.ts
export * from './useTranslation';
export * from './useSettings';
export * from './useDocumentHistory';
export * from './useMyFeature';  // 添加导出
```

### 调试技巧

1. **React DevTools**：检查组件树和状态
2. **Zustand DevTools**：监控状态变化
3. **Network Tab**：检查 API 请求
4. **Console**：查看日志输出

---

## 下一步

- 🔧 [后端开发](./04-后端开发.md) - 了解后端架构
- 🔄 [数据流设计](./05-数据流设计.md) - 理解数据流转
- 📡 [API 参考](./06-API参考.md) - 完整 API 文档
